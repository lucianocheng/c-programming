...
/*
  ...
  Payload: Last 12 bytes 
    packet[4] to packet[15] = data
                Type: chars.
*/

char packetArray[MAX_AVAILABLE_PACKETS][PACKET_SIZE];
 ...
 */
int packData(char data[], int dataSize, char packetArray[][PACKET_SIZE]){
  int packetIndex = 0;
  int j = 0;
...
char* receivePacket(){
  ...
  if(packetsReceived >= packetsSent) return NULL;
    ...
    tempPacket = (char*)packetArray[packetsReceived];
    packetsReceived++;
    return tempPacket;
  }
}
