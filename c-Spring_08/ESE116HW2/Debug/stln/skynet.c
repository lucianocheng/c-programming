...
int calculate_resistance(int max_human_age, float avg_human_age){
	if(max_human_age < 10 && avg_human_age < 5.0) return TAS_WITH_POINTY_STICKS;
	if((max_human_age > 5 && max_human_age < 10) || avg_human_age < 25) return CARE_BEARS;
	if(max_human_age * (float)avg_human_age < 30.5) return WOODY_ALLEN;
	return CHUCK_NORRIS;
}
