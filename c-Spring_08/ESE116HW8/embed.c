...
static char * fil_tr_nam;       /* string name of text input file */
static FILE * fil_tr_num;       /* file number of text input file */

static char * fil_br_nam;       /* string name of binary input file */
static FILE * fil_br_num;       /* file number of binary input file */

static char * fil_bw_nam;       /* string name of binary output file */
static FILE * fil_bw_num;       /* file number of binary output file */

/*-------------------------------------------------------------- 
| main()
|    argv[0] - char * - points to the program name            (from system)
|    argv[1] - char * - points to the text input file name    (from user)
|    argv[2] - char * - points to the binary input file name  (from user)
|    argv[3] - char * - points to the binary output file name (from user)
|
| opens three files (text input, binary input, and binary output)
-------------------------------------------------------------- */
...
int main(int argc, char * argv[])
{
   ...
   fil_tr_nam = argv[1]; /* capture text input file name */
   fil_br_nam = argv[2]; /* capture binary input file name */
   fil_bw_nam = argv[3]; /* capture binary output file name */

   if (samestring(fil_tr_nam,fil_br_nam)==1) showerror_and_quit(argv);
   if (samestring(fil_tr_nam,fil_bw_nam)==1) showerror_and_quit(argv);
   if (samestring(fil_br_nam,fil_bw_nam)==1) showerror_and_quit(argv);

   prepread(fil_tr_nam,TR,&fil_tr_num,&argv[0]);

   prepread(fil_br_nam,BR,&fil_br_num,&argv[0]);

   prepwrite(fil_bw_nam,BW,&fil_bw_num,&argv[0]);

   watermark(); /* files are all open now - YOU WRITE THIS */
...
static void closefiles()
{
   if (fil_tr_num!=NULL) if (fclose(fil_tr_num)==0); /* printf("\n1\n"); */
   if (fil_br_num!=NULL) if (fclose(fil_br_num)==0); /* printf("\n2\n"); */
   if (fil_bw_num!=NULL) if (fclose(fil_bw_num)==0); /* printf("\n3\n"); */
   return;
}
...
{
   printf("\nUsage: %s input.txt inclean.bmp outdirty.bmp\n\n",argv[0]); 
